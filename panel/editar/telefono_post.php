...
$get6_query = "SELECT * FROM quienes WHERE id=?";
$stmt = mysqli_prepare($db_connect, $get6_query);
mysqli_stmt_bind_param($stmt, "i", $id);
mysqli_stmt_execute($stmt);
$result = mysqli_stmt_get_result($stmt);
$after_assoc6 = mysqli_fetch_assoc($result);

if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['telefono'])) {
    $new_texto = $_POST['telefono'];

    $update_query6 = "UPDATE quienes SET telefono=? WHERE id=?";
    $stmt = mysqli_prepare($db_connect, $update_query6);
    ...
    <div class="bg-light mb-3 mt-3">
        ...
        <div class="card bg-light mb-3 mt-3">
            <div class="card-header bg-primary text-white text-center">
                <h4 class="text-white">Editar Telefono</h4>
            </div>
            <div class="card-body text-center">
                <form action="" method="post" enctype="multipart/form-data">
                    <div class="form-group">
                        <input type="hidden" name="id" class="form-control" value="<?= htmlspecialchars($id) ?>">
                        <input type="text" name="telefono" class="form-control" value="<?= htmlspecialchars($after_assoc6['telefono']) ?>">
                    </div>
