<?php
    ...
    $get_query = "SELECT * FROM quienes WHERE id='$id'";
    $from_db = mysqli_query($db_connect, $get_query);
    $after_assoc = mysqli_fetch_assoc($from_db);

    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $new_texto = $_POST['texto'];

    $update_query = "UPDATE quienes SET texto='$new_texto' WHERE id='$id'";
        ...
        <div class="bg-light mb-3 mt-3">
            ...
            <div class="card bg-light mb-3 mt-3">
                <div class="card-header bg-primary text-white text-center">
                    <h4 class="text-white">Editar QuienSoy</h4>
                </div>
                <div class="card-body text-center">
                    <form action="" method="post" enctype="multipart/form-data">
                        <div class="form-group">
                            <input type="hidden" name="id" class="form-control" value="<?= $after_assoc['id'] ?>">
                            <textarea name="texto" class="form-control form-control-lg" cols="45" rows="15"><?= htmlspecialchars($after_assoc['texto']) ?></textarea>
                        </div>
