...
$get5_query = "SELECT * FROM quienes WHERE id=?";
$stmt = mysqli_prepare($db_connect, $get5_query);
mysqli_stmt_bind_param($stmt, "i", $id);
mysqli_stmt_execute($stmt);
$result = mysqli_stmt_get_result($stmt);
$after_assoc5 = mysqli_fetch_assoc($result);

if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['correo'])) {
    $new_texto = $_POST['correo'];

    $update_query5 = "UPDATE quienes SET correo=? WHERE id=?";
    $stmt = mysqli_prepare($db_connect, $update_query5);
    ...
    <div class="bg-light mb-3 mt-3">
        ...
        <div class="card bg-light mb-3 mt-3">
            <div class="card-header bg-primary text-white text-center">
                <h4 class="text-white">Editar Correo</h4>
            </div>
            <div class="card-body text-center">
                <form action="" method="post" enctype="multipart/form-data">
                    <div class="form-group">
                        <input type="hidden" name="id" class="form-control" value="<?= htmlspecialchars($id) ?>">
                        <input type="text" name="correo" class="form-control" value="<?= htmlspecialchars($after_assoc5['correo']) ?>">
                    </div>
